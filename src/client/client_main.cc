...
void InitializeServers(
  ...
  for (const std::string& address : cohort_addresses) {
    if (address.substr(0, 8) == "0.0.0.0:") {
      ...
      pid_t pid = fork();
      if (pid == 0) {
        // TODO(benjmarks22): Run the start_cohort_with_blockchain_adapter
        // script. cohort::RunServer(port, /*num_db_threads=*/2,
  ...
  pid_t pid = fork();
  if (pid == 0) {
    // TODO(benjmarks22): Run the start_coordinator_with_blockchain_adapter
    // script. coordinator::RunServer(
    //     kCoordinatorPort,
    //     /*default_presumed_abort_duration=*/absl::Minutes(1));
    while (true) {
    }
  }
}
